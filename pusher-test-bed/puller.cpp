...
void Puller::findZeroPosition(){
    ...
    // Move back 1 turn
    while(motorCurrent < 0.75 && abs(currentPosition - initialPosition) < 45.0) {
    ...
    while(
        motorCurrent < 0.75 && 
        abs(currentPosition - initialPosition) < 180.0 &&
        (averageCurrent == 0 || motorCurrent < averageCurrent * 4.0 &&
        (millis() - initialTime) < 5000)
    ) {
        this->setMotorTorque(-25);
        motorCurrent = this->getMotorCurrent();
        currentPosition = this->getCurrentPosition();
...
void Puller::goToDesiredPosition() {
    // Serial.print("Going to desired pos: ");
